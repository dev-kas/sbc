...
The repository uses Node's built-in test runner. Run the full suite with:
...
The suite covers the compiler's main components:

- Utility helpers and public package exports.
- Scratch data model defaults.
- Preprocessor macros, includes, conditionals, warnings, and fatal errors.
- Lexer tokenization and parser AST construction/diagnostics.
- Analyzer behavior for scopes, sprites, lists, procedures, unreachable code,
  and opcode validation.
- Compiler output for project targets, broadcasts, control flow, procedure
  blocks, list operations, menu inputs, mutations, and assets.
- Compiler driver stage controls and `.sb3` archive generation.
- Every committed example project in `examples/`.

`npm test` also prints a coverage report. On the current suite, the local
coverage run reports 93.99% line coverage overall.
...
This README was drafted by Codex from the repository contents and may contain
mistakes or omissions. Please verify behavior against the source and tests when
accuracy matters.
